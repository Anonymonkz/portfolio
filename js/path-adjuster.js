function adjustPaths(basePath, repoName) {
      ...
      try {
        ...
        for (const rule of rules) {
          if (rule.style && rule.style.backgroundImage) {
            ...
            rule.style.backgroundImage = originalURL.replace(urlRegex, (match, url) => {
              // Adjust URL based on basePath and folder structure
              return `url(${url.replace(/^(\/|\.\/|\.\.\/|)(css|assets|pages|sub-pages|js)\/?/, `${basePath}/$2/$3`)})`;
            });
          }
        }
      } catch (error) {
  ...
  if (isGithubPages) {
    ...
    const regex = /^(\/|\.\/|\.\.\/|)(css|assets|pages|sub-pages|js)\/?([a-zA-Z0-9\-\/]*)/;

    // Adjust paths for various elements
    document.querySelectorAll('link[rel="icon"], link[rel="stylesheet"]').forEach((el) => {
